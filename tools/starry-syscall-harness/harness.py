...
def no_docker_requested(args: argparse.Namespace) -> bool:
    return bool(getattr(args, "no_docker", False)) or os.environ.get("STARRY_SYSCALL_HARNESS_NO_DOCKER") == "1"
...
def run(
    ...
    *,
    ...
    merged_env = with_local_tool_path(os.environ.copy())
    merged_env.setdefault("STARRY_SYSCALL_HARNESS_PYTHON", sys.executable)
    if env:
...
def command_exists(name: str) -> bool:
    return shutil.which(name, path=with_local_tool_path(os.environ.copy()).get("PATH")) is not None
...
def doctor(args: argparse.Namespace) -> int:
    ...
    if no_docker_requested(args):
        ...
        for name in required:
            checks.append({"name": name, "ok": command_exists(name), "path": command_path(name)})
        ...
        report = {"repo_root": str(repo_root), "mode": "local", "checks": checks}
        print(json.dumps(report, indent=2, sort_keys=True))
        required_ok = all(item["ok"] for item in checks if "optional_for" not in item)
...
def perf_profile(args: argparse.Namespace) -> int:
    repo_root = repo_root_from(Path(args.repo_root)) if args.repo_root else script_repo_root()
    if not no_docker_requested(args) and not is_inside_docker():
        forwarded = [
            "perf-profile",
            "--repo-root",
            "/work",
...
def inject_probe(rootfs: Path, probe: Path, output: Path) -> None:
    ...
    debugfs = command_path("debugfs") or "debugfs"
...
def ui(args: argparse.Namespace) -> int:
    ...
    return serve_ui(repo_root, args.host, args.port, args.image, args.open, no_docker_requested(args))
...
def main(argv: list[str] | None = None) -> int:
    ...
    doctor_parser = sub.add_parser("doctor", help="check harness prerequisites")
    doctor_parser.add_argument("--repo-root")
    doctor_parser.add_argument("--image", default=DEFAULT_IMAGE)
    doctor_parser.add_argument("--no-docker", action="store_true")
    ...
    perf_parser = sub.add_parser("perf-profile", help="run StarryOS qperf profiling")
    ...
    ui_parser.add_argument("--no-docker", action="store_true")
