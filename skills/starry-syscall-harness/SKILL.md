...
- Prefer local no-Docker execution unless the user explicitly asks for Docker.
- Pass `--no-docker` for harness commands, and point standalone kit usage at the TGOSKits checkout with `--repo-root /path/to/tgoskits`.
- Prefer the harness entrypoint over hand-built commands:
  `python3 tools/starry-syscall-harness/harness.py discover --no-docker --repo-root /path/to/tgoskits --arch riscv64`
- Prefer the qperf harness entrypoint for performance work:
  `python3 tools/starry-syscall-harness/harness.py perf-profile --no-docker --repo-root /path/to/tgoskits --arch riscv64 --timeout 20`
...
- After changing StarryOS logic, run `cargo fmt` and targeted local clippy in the TGOSKits checkout.

## Workflow

1. Run `python3 tools/starry-syscall-harness/harness.py doctor --no-docker --repo-root /path/to/tgoskits` to verify required local tools.
2. Run `python3 tools/starry-syscall-harness/harness.py discover --no-docker --repo-root /path/to/tgoskits --arch riscv64`.
...
5. Rerun the harness for the affected arch, then run local clippy for changed crates.
6. Before PR work, fetch upstream and create a clean branch from the target upstream branch.

## Performance Workflow

1. Run `python3 tools/starry-syscall-harness/harness.py perf-profile --no-docker --repo-root /path/to/tgoskits --arch riscv64 --timeout 20`.
...
8. Rerun `perf-profile`, then run local clippy for changed crates.
...
```bash
codex mcp add starry-syscall-harness -- python3 /path/to/tgoskit-harness_kit/tools/starry-syscall-harness/mcp_server.py --repo /path/to/tgoskits
```

Available tools:

- `starry_syscall_doctor`: checks local toolchain availability.
- `starry_syscall_discover`: runs Linux-vs-StarryOS syscall probes and returns the report.
- `starry_perf_profile`: runs qperf locally, emits JSON/Markdown/CSV hotspot reports, and lists fix candidates.
